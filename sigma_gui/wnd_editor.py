...
class WndEditor( pu.Wnd ):
  ...
  def m_start( self ):
    ...
    self._filename_s = pmq.request( 'm_cfg_get', 'editor_file' )
    if self._filename_s is not None:
      if os.path.isfile( self._filename_s ):
        pmq.post( 'm_fopen', self._filename_s )
      else:
  ...
  def m_shutdown( self ):
    ...
    if self._filename_s is not None:
      pmq.post( 'm_cfg_set', 'editor_file', self._filename_s )
  ...
  def _updateCaption( self, s_file = None ):
    if s_file is None:
  ...
  def m_project_file_set( self, s_subpath ):
    ...
    if oProject is not None:
      self.m_fopen( os.path.join( oProject.dir, s_subpath ) )
  ...
  def m_fopen( self, s_filename ):
    try:
      with open( s_filename ) as oFile:
        ...
        self._updateCaption( s_filename )
        self._filename_s = s_filename
