...
class EditorVim( pmq.Actor ):
  ...
  def m_editor_geometry_get( self ):
    if self._use_f:
      pmq.response( self._editorGeometry_g )
  ...
  def m_startup( self ):
    ...
    self._editorGeometry_g = self._wndGeometry()


  def _wndGeometry( self ):
    if sys.platform == 'win32':
      @ctypes.WINFUNCTYPE( ctypes.c_int, ctypes.c_int, ctypes.c_int )
      def enumWindowsCallback( i_hWindow, i_nBaton ):
        with EditorVim._guard_o:
          if i_nBaton in EditorVim._objects_m:
            nId = int( i_nBaton )
            EditorVim._objects_m[ nId ].enumWindowsCallback( i_hWindow )
        return 1
      ##  VIM window handle.
      self._window_h = None
      nId = id( self )
      with EditorVim._guard_o:
        EditorVim._objects_m[ nId ] = self
      ##  |enumWindowsCallback()| will be called for each window.
      ctypes.windll.user32.EnumWindows( enumWindowsCallback, nId )
      if self._window_h is not None:
        oRect = WinapiRect()
        pRect = ctypes.pointer( oRect )
        ctypes.windll.user32.GetWindowRect( self._window_h, pRect )
